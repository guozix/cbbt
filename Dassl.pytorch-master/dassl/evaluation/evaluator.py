...
@EVALUATOR_REGISTRY.register()
class SL_Classification(EvaluatorBase):
    """Evaluator for classification."""
    ...
    def evaluate(self):
        ...
        targets = torch.cat(self._y_true, dim=0)
        ...
        if self.activate_func == 'default_merge_aux':
            ...
            preds_ = preds.max(1)[1]
            matches = preds_.eq(targets).float()
            _correct = int(matches.sum().item())
            _total = targets.shape[0]
            print("ACC default:", 100.0 * _correct / _total)
            _score = 100.0 * _correct / _total
            
            if len(self._y_pred_aux) > 0:
                preds_aux_ = preds_aux.max(1)[1]
                matches = preds_aux_.eq(targets).float()
                _correct = int(matches.sum().item())
                _total = targets.shape[0]
                print("ACC aux:", 100.0 * _correct / _total)
                
                for i in np.linspace(0.0, 1.0, 11):
                    preds_ = (preds * (1-i) + preds_aux * i).max(1)[1]  # (preds_sigmoid + preds_softmax) / 2
                    matches = preds_.eq(targets).float()
                    _correct = int(matches.sum().item())
                    _total = targets.shape[0]
                    acc = 100.0 * _correct / _total
                    print("ACC merged ()+({}):".format(i), acc)

                    # if (i-0.5) < 1e-5:
                    #     _score = acc
            
        else:
            raise NotImplementedError('Not imple. in SL_Classification...')
        print("ACC:", _score)
...
class MLClassification(EvaluatorBase):
    ...
    def evaluate(self):
        ...
        if self.activate_func == 'sigmoid':
        ...
        elif self.activate_func == 'merge_ce_bce':
            preds_sigmoid = torch.sigmoid(preds).cpu().numpy()
            mAP_score_sigmoid = mAP(targets, preds_sigmoid)
            print("mAP score sigmoid:", mAP_score_sigmoid)

            preds_softmax = torch.nn.functional.softmax((preds), dim=1).cpu().numpy()
            # pred = torch.nn.functional.softmax(mo * 50, dim=1)
            mAP_score_softmax = mAP(targets, preds_softmax)
            print("mAP score softmax:", mAP_score_softmax)

            mAP_score = mAP(targets, preds.cpu().numpy())
            print("mAP score none:", mAP_score)

            for i in np.linspace(0, 1, 6):
                preds_merge = (preds_sigmoid * (1-i) + preds_softmax * i)  # (preds_sigmoid + preds_softmax) / 2
                mAP_score = mAP(targets, preds_merge)
                print("mAP score merged : preds_sigmoid + preds_softmax(w{}):".format(i), mAP_score)

        elif self.activate_func == 'merge_aux':
            preds_sigmoid = torch.sigmoid(preds).cpu().numpy()
            mAP_score_sigmoid = mAP(targets, preds_sigmoid)
            print("mAP score sigmoid:", mAP_score_sigmoid)

            preds_softmax_ = torch.nn.functional.softmax(preds, dim=1).cpu().numpy()
            # pred = torch.nn.functional.softmax(mo * 50, dim=1)
            mAP_score_softmax = mAP(targets, preds_softmax_)
            print("mAP score dualp softmax:", mAP_score_softmax)

            preds_softmax = torch.nn.functional.softmax(preds_aux, dim=1).cpu().numpy()
            # pred = torch.nn.functional.softmax(mo * 50, dim=1)
            mAP_score_softmax = mAP(targets, preds_softmax)
            print("mAP score softmax:", mAP_score_softmax)

            preds__ = preds.cpu().numpy()
            mAP_score = mAP(targets, preds__)
            print("mAP score none:", mAP_score)

            for i in np.linspace(0.0, 1.0, 21):
                preds_merge = (preds_sigmoid * (1-i) + preds_softmax * i)  # (preds_sigmoid + preds_softmax) / 2
                mAP_score_ = mAP(targets, preds_merge)
                print("mAP score merged : preds_sigmoid + preds_softmax(w{}):".format(i), mAP_score_)
            for i in np.linspace(0.6, 1.0, 21):
                preds_merge = (preds__ * (1-i) + preds_softmax * i)  # (preds_sigmoid + preds_softmax) / 2
                mAP_score_ = mAP(targets, preds_merge)
                print("mAP score merged : preds_ranking + preds_softmax(w{}):".format(i), mAP_score_)
            for i in np.linspace(0.6, 1.0, 21):
                preds_merge = (preds_softmax_ * (1-i) + preds_softmax * i)  # (preds_sigmoid + preds_softmax) / 2
                mAP_score_ = mAP(targets, preds_merge)
                print("mAP score merged : preds_softmax_dual + preds_softmax(w{}):".format(i), mAP_score_)

        elif self.activate_func == 'merge_gcn_rank_bce': 
            preds_sigmoid = torch.sigmoid(preds * 10).cpu().numpy()
            mAP_score_sigmoid = mAP(targets, preds_sigmoid)
            print("mAP score sigmoid:", mAP_score_sigmoid)

            preds_softmax_ = torch.nn.functional.softmax(preds * 40, dim=1).cpu().numpy()
            # pred = torch.nn.functional.softmax(mo * 50, dim=1)
            mAP_score_softmax = mAP(targets, preds_softmax_)
            print("mAP score dualp softmax:", mAP_score_softmax)

            preds_rank = preds_aux.cpu().numpy()
            # pred = torch.nn.functional.softmax(mo * 50, dim=1)
            mAP_score_rank = mAP(targets, preds_rank)
            print("mAP score rank:", mAP_score_rank)
            ...
            for i in np.linspace(0.0, 1.0, 21):
                preds_merge = (preds_sigmoid * (1-i) + preds_rank * i)  # (preds_sigmoid + preds_softmax) / 2
                mAP_score_ = mAP(targets, preds_merge)
                print("mAP score merged : preds_sigmoid + preds_rank(w{}):".format(i), mAP_score_)
            # for i in np.linspace(0.6, 1.0, 21):
            #     preds_merge = (preds__ * (1-i) + preds_softmax * i)  # (preds_sigmoid + preds_softmax) / 2
            #     mAP_score_ = mAP(targets, preds_merge)
            #     print("mAP score merged : preds_ranking + preds_softmax(w{}):".format(i), mAP_score_)
            ...
            preds_ = torch.nn.functional.softmax(preds * 2, dim=1).cpu().numpy()
            # pred = torch.nn.functional.softmax(mo * 50, dim=1)
            mAP_score_softmax = mAP(targets, preds_)
            print("mAP score softmax:", mAP_score_softmax)
            ...
            if len(self._y_pred_aux) > 0:
                preds_aux_ = torch.sigmoid(preds_aux).cpu().numpy()
                mAP_score_sigmoid_aux = mAP(targets, preds_aux_)
                print("mAP score sigmoid aux:", mAP_score_sigmoid_aux)
                
                preds_softmax_aux_ = torch.nn.functional.softmax(preds_aux * 2, dim=1).cpu().numpy()
                mAP_score_softmax_aux = mAP(targets, preds_softmax_aux_)
                print("mAP score softmax aux:", mAP_score_softmax_aux)

                mAP_score_aux = mAP(targets, preds_aux.cpu().numpy())
                print("mAP score aux:", mAP_score_aux)
                
                for i in np.linspace(0.0, 1.0, 21):
                    preds_merge = (preds.cpu().numpy() * (1-i) + preds_aux.cpu().numpy() * i)  # (preds_sigmoid + preds_softmax) / 2
                    mAP_score_ = mAP(targets, preds_merge)
                ...
                for i in np.linspace(0.0, 1.0, 21):
                    preds_merge = (preds * (1-i) + preds_aux * i)  # (preds_sigmoid + preds_softmax) / 2
                    preds_merge = torch.nn.functional.softmax(preds_merge * 2, dim=1).cpu().numpy()
                    mAP_score_ = mAP(targets, preds_merge)
                    print("mAP score merged : preds_sigmoid + preds_softmax(w{}):".format(i), mAP_score_)
                    
        elif self.activate_func == 'dualcoop':
            ...
            if len(self._y_pred_aux) > 0:
                ...
                preds_softmax_aux_ = torch.nn.functional.softmax(preds_aux, dim=1).cpu().numpy()
                mAP_score_softmax_aux = mAP(targets, preds_softmax_aux_)
                print("mAP score softmax aux:", mAP_score_softmax_aux)

                mAP_score_aux = mAP(targets, preds_aux.cpu().numpy())
                print("mAP score aux:", mAP_score_aux)
                mAP_score = mAP_score_aux
                ...
                for i in np.linspace(0.0, 1.0, 21):
                    preds_merge = (preds * (1-i) + preds_aux * i)  # (preds_sigmoid + preds_softmax) / 2
                    preds_merge = torch.nn.functional.softmax(preds_merge, dim=1).cpu().numpy()
                    mAP_score_ = mAP(targets, preds_merge)
                    print("mAP score merged : preds_sigmoid + preds_softmax(w{}):".format(i), mAP_score_)
        ...
        # with open('tmp_preds_partial_nus_0_8.pkl', 'wb') as f:
